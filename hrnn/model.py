...
class HRNNModel(object):
    def __init__(self,
                 ...
                 seq_len,
                 emb_size):
        self.batch_size = batch_size # Number of examples per batch
        self.big_frame_size = big_frame_size # Number of samples per big frame
        self.frame_size = frame_size # Number of samples per frame
        self.q_levels = q_levels # Number of bins for quantization of samples
        self.rnn_type = rnn_type # RNN cell type
        self.dim = dim # State size of RNN cells
        self.n_rnn = n_rnn # Number of layers in stacked RNN {1, 2, 3, 4, 5}
        self.seq_len = seq_len # Number of samples to include in Truncated BPTT pass
        self.emb_size = emb_size # Size of embedding layer (> 0)
    ...
    def _create_network_BigFrame(self,
                                 ...
                                 big_frame_state,
                                 big_input_sequences):
        with tf.variable_scope('BigFrame_layer', reuse=tf.AUTO_REUSE):
    ...
    def _create_network_Frame(self,
                              ...
                              frame_state,
                              input_sequences):
        with tf.variable_scope('Frame_layer', reuse=tf.AUTO_REUSE):
    ...
    def _create_network_Sample(self,
        ...
        with tf.variable_scope('Sample_layer', reuse=tf.AUTO_REUSE):
            ...
            sample_input_sequences = tf.reshape(sample_input_sequences, sample_shape)
            ...
            sample_filter = tf.get_variable('sample_filter',
                                            sample_filter_shape,
            ...
            out = tf.reshape(out,
                             [-1, sample_shape[1]/self.emb_size - 1, self.q_levels])
            return out

    def _create_network_HRNN(self,
                             train_big_frame_state,
                             train_frame_state):
        with tf.name_scope('HRNN_net'):
            # Big frame
    ...
    def loss_HRNN(self,
        ...
        with tf.name_scope(name):
            ...
            wb_encoded_rnn = self._one_hot(self.encoded_wb_input_rnn)
            raw_output, final_big_frame_state, final_frame_state = \
                self._create_network_HRNN(train_big_frame_state, train_frame_state)
            with tf.name_scope('loss'):
                target_output_rnn = wb_encoded_rnn[:, self.big_frame_size:, :]
                print(target_output_rnn) # (16, 512, 256)
                target_output_rnn = tf.reshape(target_output_rnn,
                                               [-1, self.q_levels])
                prediction = tf.reshape(raw_output, [-1, self.q_levels])
                print(target_output_rnn) # (8192, 256)
                print(prediction) # (?, 256)
